...
export async function getStaticProps(context) {
  ...
  const request1 = await fetch(
    `${process.env.webURL}/Comment/GetSoundComment?CourseId=${paths}&page=1&pagesize=5`
  );
 try{
  ...
  const datafunc = async (p)=>{
    try {
      const result = await fetch(
        `${process.env.webURL}/Comment/GetSoundComment?CourseId=${cd.id}&page=${p}&pagesize=5`
        );
      ...
      <div className={styles.container}>
        ...
        src={cd.filePath}>
            ...
            <div className={`col-xl-5 ${detail.session} ${detail.soundsession}`}>
              ...
              {cd.relatedItems.map((i,index)=>{
                return(
                  <div className={`col-11 mx-auto ${detail.sounditem}`} key={index}>
                <div className={`row`}>
                    ...
                    <div className={`${detail.speechertime}`}>{clock}<span className={``}>{i.duration}</span>
                    </div>
                    </div>
              </div>
